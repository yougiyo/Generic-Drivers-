...
The drivers were developed and tested on the **STM32 Nucleo-F401RE** development board, using the **STM32F401RE** microcontroller (**ARM Cortex-M4**).

Other STM32 devices may also work, but you should verify register layout, clock tree, interrupt assignments, and CMSIS/device-header assumptions before use.
...
All five current drivers—**FPU, UART, timebase, GPIO, and ADC**—are implemented and functional, and were tested on the **STM32 Nucleo-F401RE**.

This remains an educational and extensible bare-metal driver collection; review the target MCU reference manual and datasheet before production deployment.
